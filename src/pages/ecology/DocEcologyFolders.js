		...
		<div className="container">
			{
				dirs?.map((el, idx) => {
					if (el.dirs.length) {
						return (
							<NavLink exact to={`/ecology/folders/${el.id}/nest/1`} onClick={() => {
						...
						return (
							<NavLink exact to={`/ecology/list/${el.id}`} onClick={() => {
								dispatch(setDir(el.id))
							}} className="doc-folder" key={idx}>
								<img src={folderIcon} alt="folder-icon" />
								<p className="doc-folder__name">{el.name}</p>
...
export default DocEcologyFolders;