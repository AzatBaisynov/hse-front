...
const PnbvFactLoggingYear = () => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      if (file?.name) {
        ...
        try {
          await axios.post(
            `${_LINK}/v1/api/file/excel?type=13&date=2022-10-12`,
            formData,
            {
              headers: {},
            }
  ...
  useEffect(() => {
    const getArray = async () => {
      const config = {
        method: "get",
        url: `${_LINK}/v1/api/file/excel/get?type=13`,
        headers: {
  ...
  const title = `Незапланированные записи по факту ПНБВ на ${year} год`;
...
export default PnbvFactLoggingYear;