...
const PnbvScheduleMonth = () => {
  const [file, setFile] = useState({});
  const [excel, setExcel] = useState({});const [document, setDocument] = useState({});
  ...
  const handleSend = async (e) => {
    ...
    try {
      if (file?.name) {
        ...
        try {
          await axios.post(
            `${_LINK}/v1/api/file/excel?skip=4&cols=0&type=11&date=2022-10-12`,
            formData,
            {
              headers: {},
            }
  ...
  useEffect(() => {
    const getArray = async () => {
      const config = {
        method: "get",
        url: `${_LINK}/v1/api/file/excel/get?type=11`,
        headers: {
      ...
      <div className="doc-list__content">
        ...
        <div className="doc-list__head-panel">
          ...
          <div className="doc-list__buttons">
            ...
            {/* CHOOSE DATE PERIOD */}
            <input type="month" onInput={handleInput}/>
          </div>
        ...
        <table className="ensk-table">
          <thead>
            <tr>
              ...
              <th>Должность</th>
              <th>Ф.И.О.</th>
              <th>Пн - Вс <br/> 1 неделя </th>
              <th>Пн - Вс <br/> 2 неделя </th>
              <th>Пн - Вс <br/> 3 неделя </th>
              <th>Пн - Вс <br/> 4 неделя </th>
              <th>Пн - Вс <br/> 5 неделя </th>
            </tr>
          </thead>
          {createTable()}
        </table>
...
export default PnbvScheduleMonth;
