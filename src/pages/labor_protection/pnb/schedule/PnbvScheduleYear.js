...
const PnbvScheduleYear = () => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      if (file?.name) {
        ...
        try {
          await axios.post(
            `${_LINK}/v1/api/file/excel?type=10&skip=4&cols=0&date=2022-10-12`,
            formData,
            {
              headers: {},
            }
  ...
  useEffect(() => {
    const getArray = async () => {
      const config = {
        method: "get",
        url: `${_LINK}/v1/api/file/excel/get?type=10`,
        headers: {
  ...
  const title = `План график ПНБВ на ${year} год`;
...
export default PnbvScheduleYear;
